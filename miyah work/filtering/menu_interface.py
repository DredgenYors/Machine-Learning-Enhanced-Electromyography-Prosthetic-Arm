...
from .io_utils import load_matlab_file, batch_process_zip_of_mat_files
from .plots import (
...
def _display_menu():
    ...
    print("20. Load new .mat file")
    print("21. Batch process ZIP of .mat files → combined CSV")
    print("0. Exit")

def run_cli():
    state = PipelineState(fs=1000)
    if not load_matlab_file(state):
    ...
    while True:
        ...
        if choice == 0:
            ...
            load_matlab_file(state)

        elif choice == 21:
            zp = input("Path to ZIP of .mat files: ").strip()
            out = input("Output CSV (default combined_emg_features.csv): ").strip() or "combined_emg_features.csv"
            batch_process_zip_of_mat_files(zp, state, out)
